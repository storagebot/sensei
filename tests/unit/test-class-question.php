...
class Sensei_Class_Question_Test extends WP_UnitTestCase {
    ...
    public function testClassInstance() {
        ...
        $this->assertTrue( isset( $woothemes_sensei->question ), 'Sensei Question class is not loaded' );
    ...
    public function testGetQuestionType() {
        ...
        $should_be_false = $woothemes_sensei->question->get_question_type('');
        ...
        $this->assertFalse( $should_be_false ,
            'The method get_question_type should return false for an empty string parameter');
        $should_be_false = $woothemes_sensei->question->get_question_type( 2000 );
        $this->assertFalse( $should_be_false ,
            'The method get_question_type should return false for an empty string parameter');

        // does this method return a string for a valid question id
        $questions = get_posts( 'post_type=question' );
        $should_be_question_type = $woothemes_sensei->question->get_question_type( $questions[  array_rand( $questions ) ]->ID );
        $sensei_question_types =   array_keys( $woothemes_sensei->question->question_types() );
